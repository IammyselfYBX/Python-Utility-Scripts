...
def extract_comments(file_content):
    """
    提取 LaTeX 文件中的注释：
    - 匹配所有以 % 开头的注释，或 % 前有空格/TAB 的注释。
    """
    # 匹配单行注释，包括行尾注释和行首注释
    single_line_pattern = r"(?m)^\s*%.*$|(?<=\s)%.*$"

    # 提取所有匹配的注释
    comments = re.findall(single_line_pattern, file_content)

    # 返回所有非空注释
    return [comment for comment in comments if comment.strip()]
...
if __name__ == "__main__":
    input_file = input("请输入要处理的 LaTeX 文件路径： ")
